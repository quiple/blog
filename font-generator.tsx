...
import {$Bitmap, $Font, type DirectionType, type Glyph, type GlyphMeta} from 'bdfparser'
import fetchline from 'fetchline'
import hangul from './generator/hangul'
import hanja from './generator/hanja'
import japanese from './generator/japanese'
import kanji from './generator/kanji'
import korean from './generator/korean'
import pageStyle from './generator/style.scss?inline'
...
const getFontUrl = (font: string): string => `https://cdn.jsdelivr.net/${fontUrl[font]}` || ''
...
export default component$(() => {
  ...
  const drawFontImage = $(async (event: SubmitEvent, target: HTMLFormElement) => {
    const formData = new FormData(target)
    const font = formData.get('font') as string
    const charset = formData.get('charset') as string
    const xOffset = Number(formData.get('x-offset'))
    const yOffset = Number(formData.get('y-offset'))
    const tileWidth = Number(formData.get('tile-width'))
    const tileHeight = Number(formData.get('tile-height'))
    const tileColumn = Number(formData.get('tile-column'))
    const foreground = formData.get('foreground') as string
    const background = formData.get('background') as string
    const shadowColor = formData.get('shadow-color') as string
    ...
    if (includesArray(position, [-1, -1]) || includesArray(position, [-1, 0]) || includesArray(position, [-1, 1])) {
      xOff++
    }
    if (includesArray(position, [-1, 1]) || includesArray(position, [0, 1]) || includesArray(position, [1, 1])) {
      yOff++
    }
  ...
  return (
    <main class="container mx-auto flex flex-col-reverse lg:flex-row items-start gap-[20px] p-[20px]">
      <aside class="sticky top-[20px] flex lg:w-[24rem] flex-col gap-[20px]">
        <form class="flex flex-col gap-[10px]" preventdefault:submit onSubmit$={drawFontImage}>
          <div class="form">
            <div class="form-row">
              ...
              <div class="right">
                ...
                >
                  ...
                  <optgroup label="Num Kadoma">
                    ...
                    <option value="misaki-gothic-2nd" label="Misaki Gothic 2nd (8px)" />
            ...
            {fontCurrent.value === 'custom' && (
              <div class="form-row col">
                ...
                <div class="right">
                  <input type="file" id="custom-font" name="custom-font" accept=".bdf" />
                </div>
              </div>
            )}
            <div class="form-row">
              ...
              <div class="right">
                ...
                >
                  ...
                  <optgroup label="EUC-KR">
                    ...
                    <option value="euckrWoHanja2355" label="한자 제외, 한글 2355자" />
                    <option value="euckrWoHanja2780" label="한자 제외, 한글 2780자" />
                    <option value="euckrWoHanja4358" label="한자 제외, 한글 4358자" />
                    <option value="euckrWoHanja11172" label="한자 제외, 한글 11172자" />
                  </optgroup>
                  <optgroup label="Shift_JIS">
                    <option value="shiftjis" label="Shift_JIS" />
                    <option value="shiftjis_level1" label="제1수준 한자만 포함" />
                  </optgroup>
                  <optgroup>
              ...
              <div class="form-row col">
                ...
                <div class="right">
                  <textarea
                    ...
                    lang={
                      ['jis2965', 'jis6355', 'unicode2965', 'unicode6355', 'shiftjis', 'shiftjis_level1'].some(
                        (charset) => charsetCurrent.value.startsWith(charset),
                      )
                        ? 'ja'
                        : 'ko'
                    }
            ...
            <div class="form-row">
              ...
              <div class="right">
                <div class="!grid grid-cols-3 !gap-0">
                  <input type="checkbox" name="shadow-topleft" value="-1 1" id="shadow-topleft" />
                  <input type="checkbox" name="shadow-top" value="0 1" id="shadow-top" />
                  <input type="checkbox" name="shadow-topright" value="1 1" id="shadow-topright" />
                  <input type="checkbox" name="shadow-left" value="-1 0" id="shadow-left" />
                  <input type="checkbox" disabled />
                  <input type="checkbox" name="shadow-right" value="1 0" id="shadow-right" />
                  <input type="checkbox" name="shadow-bottomleft" value="-1 -1" id="shadow-bottomleft" />
                  <input type="checkbox" name="shadow-bottom" value="0 -1" id="shadow-bottom" />
                  <input type="checkbox" name="shadow-bottomright" value="1 -1" id="shadow-bottomright" />
                </div>
              </div>
            </div>
          </div>
        ...
        <article class="prose prose-sm prose-zinc max-w-full">
          ...
          <ul>
            <li>
              ...
              <ul>
                ...
                <li>
                  2780자: <ExternalLink text="Adobe-KR-0" href="https://github.com/adobe-type-tools/Adobe-KR" />의 모든
                  한글 음절. KS X 1001의 2350자를 포함합니다.
                </li>
                <li>
                  4358자: Adobe-KR-0과 Adobe-KR-1의 모든 한글 음절. KS X 1001,{' '}
                  <ExternalLink text="KS X 1002" href="https://en.wikipedia.org/wiki/KS_X_1002" />,{' '}
                  <ExternalLink text="KPS 9566" href="https://www.unicode.org/L2/L2018/18011-info-kps9566-2011.pdf" />,{' '}
                  <ExternalLink
                    text="GB/T 12052"
          ...
          <p>&copy; 2024 Lee Minseo. 각 폰트는 해당 소유자, 저작권자 및 사용 허가자의 상표 및 저작권 자산입니다.</p>
        </article>
      </aside>
      <div class="flex flex-1 items-center justify-center self-stretch bg-zinc-50 min-h-40 rounded-md">
        ...
        {drawButtonDisabled.value ? (
          ...
          <span class="canvas-placeholder m-4">폰트 이미지를 만들려면 조건을 설정하고 만들기 버튼을 누르세요.</span>
        )}
      </div>
    </main>
  )
