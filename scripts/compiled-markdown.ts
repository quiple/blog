  ...
  return (tree: Root) => {
    visit(tree, (node) => {
      ...
      if (node.name === 'figure') {
        ...
        content = `<iframe class="${cn('mdx-embed-frame w-full rounded-lg shadow-xs mx-auto self-center', className)}" data-testid="embed-iframe" src="https://open.spotify.com/embed/${id?.replace(':', '/')}?locale=ko" width="100%" height="352" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy" decoding="async"></iframe>`
      } else {
