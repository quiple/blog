...
export const blogPostMetadataSchema = v.object({
  ...
  thumbnail: v.optional(v.pipe(v.string(), v.trim())),
...
export function getArticleMetadataFromMatter(category: string, slug: string, data: {[key: string]: unknown}) {
  const article = v.parse(blogArticleMetadataSchema, {
    category,
    slug,
    ...data,
  })
  const canonicalURL = new URL(`/${article.category}/${article.slug}`, BASE_URL).toString()
  const relativeURL = `/${article.category}/${article.slug}`

  return {...article, canonicalURL, relativeURL}
}
...
export function getBlogPostsMetadata() {
  const posts = Object.entries(blogPosts)
    .map(([filePath, rawContent]) => {
      ...
      return getPostMetadataFromMatter(category, slug, data)
    })
...
export function getBlogArticlesMetadata() {
  const articles = Object.entries(blogArticles)
    .map(([filePath, rawContent]) => {
      ...
      return getArticleMetadataFromMatter(category, slug, data)
    })
    .sort((a, b) => Date.parse(`${b.pubDate.valueOf()}+09:00`) - Date.parse(`${a.pubDate.valueOf()}+09:00`))
