...
현재 디렉토리의 `city-girl.ttml`에서 행·음절 타이밍을 읽어 `src/posts/lyric/city-girl.yaml`에 이식합니다. 원문·발음·번역·메타데이터는 유지하며, 원본이 음절별이면 기존 `text`를 같은 원문의 `words`로 나눕니다. `time`은 초 단위의 `[시작, 종료]`로 저장합니다.

행 수나 단어 구분이 맞지 않으면 파일을 변경하지 않고 중단합니다. 원문 차이만 무시하고 대응하는 행·단어에 적용하려면 `--force`를 붙입니다. 타이밍이 없는 TTML 행은 이식할 수 없습니다.
